...
namespace TKSR
{
	...
	public class TKSRChatAnchor : MonoBehaviour
	{
		[Tooltip("Chat Bubble that should be anchored at this transform.")]
		public TKSRChatBubble AttachedBubble;
		public bool BindAttachedBubble( TKSRChatBubble value )
		{
		...
		void OnEnable()
		{
			...
			m_parentSprite = this.GetComponentInParent<SpriteRenderer>(true);
		...
		/// <summary>
		/// Story运行过程中根据人物朝向,调整Anchor的朝向
		/// </summary>
		private void UpdateAnchorFlip()
		{
			if (m_parentSprite != null)
			{
				var anchorPos = m_selfTransform.localPosition;
				if (m_parentSprite.flipX)
				{
					anchorPos.x = -Mathf.Abs(anchorPos.x);
				}
				else
				{
					anchorPos.x = Mathf.Abs(anchorPos.x);
				...
				m_selfTransform.localPosition = anchorPos;
			}
		}
