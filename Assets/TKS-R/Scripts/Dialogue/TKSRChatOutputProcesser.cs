...
namespace TKSR
{
	...
	public class TKSRChatOutputProcesser : MonoBehaviour
	{
		...
		TKSRChatBubble _chatBubble;
		TKSRChatBubble chatBubble { get { if (!_chatBubble) _chatBubble = GetComponent<TKSRChatBubble>(); return _chatBubble; } }
		...
		// [TKSR] 关联ChatOutput和对应的UI Panel
		public TKSRChatBubbleSubtitlePanel parentChatPanel;
		...
		IEnumerator Process()
		{
			...
			if (TimelineScenarioItem.s_IsDialogAutoInTimeline)
			{
				...
				{
					parentChatPanel.ContinueWhenChatFinished();
				}
			...
			if (Profile.CompletionClick)
			{
				...
				while ((!Input.GetMouseButtonUp(0) && !Input.GetKeyUp(KeyCode.Return) && !Input.GetKeyUp(KeyCode.Space) ) || parentChatPanel.filterContinueByUI)
					yield return null;
#elif UNITY_ANDROID || UNITY_IOS
				while (Input.touchCount == 0 || parentChatPanel.filterContinueByUI)
					yield return null;
#endif
			}
			else
