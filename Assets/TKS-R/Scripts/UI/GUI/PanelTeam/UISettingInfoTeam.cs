...
namespace TKSR
{
    public class UISettingInfoTeam : MonoBehaviour
    {
        public UITeamTypeTab[] btnTabs;
        public List<UITeamSubContent> contentPanels;
        ...
        #region Tab Buttons
        private UITeamTypeTab m_selectedTab;
        private void OnTabButtonClicked(UITeamTypeTab btnTab)
        {
        ...
        private void InitAllTabsStatus()
        {
            foreach (var tab in btnTabs)
            {
                if (tab.tabType == EnumUITeamType.Group)
                {
                    m_selectedTab = tab;
                    break;
                }
